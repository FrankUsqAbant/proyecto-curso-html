...
gain.connect(audioCtx.destination);
osc.start();
osc.stop();`},tests:[{desc:"Instancia un AudioContext",test:e=>/new\s*\(?\s*(window\.)?AudioContext/i.test(e.js)},{desc:"Crea un oscilador con createOscillator()",test:e=>/createOscillator\(/i.test(e.js)},{desc:"Inicia el sonido con start()",test:e=>/\.start\(/i.test(e.js)}]}];class m{static evaluate(a,t=[]){if(!t||t.length===0)return{passed:!0,score:100,results:[{desc:"Reto de práctica libre",passed:!0}]};const s=t.map(n=>{try{const r=n.test(a);return{desc:n.desc,passed:!!r}}catch(r){return{desc:n.desc,passed:!1,error:r.message}}}),o=s.filter(n=>n.passed).length,i=Math.round(o/s.length*100);return{passed:o===s.length,score:i,results:s,passedCount:o,totalCount:s.length}}}class b{constructor(a){this.container=a,this.activeTab="html",this.code={html:"",css:"",js:""},this.debounceTimer=null,this._renderShell()}setCode(a,t=!0){this.code={html:a.html||"",css:a.css||"",js:a.js||""},this.updateEditorContent(),t&&this.runPreview()}getCode(){return{...this.code}}_renderShell(){this.container.className="sandbox-container",this.container.innerHTML=`
    ...
    `,this.container.querySelectorAll(".code-tab-btn").forEach(s=>{s.addEventListener("click",()=>{const o=s.getAttribute("data-tab");this.switchTab(o)})});const a=this.container.querySelector("#sandbox-textarea");a.addEventListener("input",s=>{this.code[this.activeTab]=s.target.value,clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>{this.runPreview()},600)}),a.addEventListener("keydown",s=>{if(s.key==="Tab"){s.preventDefault();const o=a.selectionStart,i=a.selectionEnd;a.value=a.value.substring(0,o)+"  "+a.value.substring(i),a.selectionStart=a.selectionEnd=o+2,this.code[this.activeTab]=a.value}}),this.container.querySelector("#sandbox-run-btn").addEventListener("click",()=>{this.runPreview()})}switchTab(a){const t=this.container.querySelector("#sandbox-textarea");this.code[this.activeTab]=t.value,this.activeTab=a,this.container.querySelectorAll(".code-tab-btn").forEach(s=>{s.classList.toggle("active",s.getAttribute("data-tab")===a)}),this.updateEditorContent()}updateEditorContent(){const a=this.container.querySelector("#sandbox-textarea");a.value=this.code[this.activeTab]||""}runPreview(){const a=this.container.querySelector("#sandbox-iframe"),t=this.container.querySelector("#console-logs-list");t.innerHTML="";const o=`
      <!DOCTYPE html>
      <html lang="es">
      <head>
        <meta charset="UTF-8">
    ...
    `;a.srcdoc=o}}const g=[{title:"Centrado Perfecto en 1 Línea (CSS)",category:"CSS",code:`display: grid;
...
}`}];function v(){const e=document.getElementById("snippets-modal-overlay");e&&e.remove();const a=document.createElement("div");a.id="snippets-modal-overlay",a.className="modal-overlay",a.innerHTML=`
        ...
        ${g.map((t,s)=>`
          <div style="background: var(--bg-app); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 0.9rem;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
              <span style="font-weight: 700; color: #ffffff; font-size: 0.85rem;">${t.title}</span>
            ...
            <pre style="margin: 0; background: #06080e; padding: 0.65rem; border-radius: 6px; font-family: var(--font-mono); font-size: 0.78rem; color: #38bdf8; overflow-x: auto;"><code>${t.code}</code></pre>
          </div>
        `).join("")}
      </div>
    </div>
  `,document.body.appendChild(a),a.querySelector("#close-modal-btn").addEventListener("click",()=>{a.remove()}),a.addEventListener("click",t=>{t.target===a&&a.remove()})}class h{constructor(){this.currentView="home",this.currentGuardian="html",this.currentLessonIndex=0,this.sandbox=null,this.completedLessons=new Set(JSON.parse(localStorage.getItem("devfauna_completed")||"[]")),this.showHint=!1,this.lastTestResult=null,this.init()}get lessonsMap(){return{html:d,css:l,js:c}}get currentLessons(){return this.lessonsMap[this.currentGuardian]||d}get activeLesson(){return this.currentLessons[this.currentLessonIndex]||this.currentLessons[0]}get totalLessonsCount(){return d.length+l.length+c.length}get overallProgressPercent(){return Math.round(this.completedLessons.size/this.totalLessonsCount*100)}init(){document.getElementById("app")&&(window.addEventListener("message",t=>{if(t.data&&t.data.type==="DEVFAUNA_LOG"){const s=document.getElementById("console-logs-list");if(s){const o=document.createElement("div");o.className="console-entry",o.textContent=`> ${t.data.text}`,s.appendChild(o)}}}),this.render())}render(){const a=document.getElementById("app");a&&(a.innerHTML=`
      <!-- 1. Header Global -->
    ...
    `,document.getElementById("global-home-trigger").addEventListener("click",()=>{this.switchView("home")}),document.getElementById("header-snippets-btn").addEventListener("click",()=>{v()}),this.renderCurrentView())}switchView(a,t=null,s=0){this.currentView=a,t&&(this.currentGuardian=t,this.currentLessonIndex=s),this.showHint=!1,this.lastTestResult=null,this.renderCurrentView()}renderCurrentView(){const a=document.getElementById("views-viewport");a&&(this.currentView==="home"?this.renderHomeView(a):this.renderLabView(a))}renderHomeView(a){const t=d.filter(i=>this.completedLessons.has(i.id)).length,s=l.filter(i=>this.completedLessons.has(i.id)).length,o=c.filter(i=>this.completedLessons.has(i.id)).length;a.innerHTML=`
          ...
          <div class="guardians-grid-pavilions">
            ...
            <div class="pavilion-card html">
              ...
              <div class="pavilion-footer">
                <span class="pavilion-stat">${t} / ${d.length} Completados</span>
            ...
            <div class="pavilion-card css">
              ...
              <div class="pavilion-footer">
                <span class="pavilion-stat">${s} / ${l.length} Completados</span>
            ...
            <div class="pavilion-card js">
              ...
              <div class="pavilion-footer">
                <span class="pavilion-stat">${o} / ${c.length} Completados</span>
    ...
    `,a.querySelectorAll(".enter-pavilion-btn").forEach(i=>{i.addEventListener("click",()=>{const n=i.getAttribute("data-guardian");this.switchView("lab",n,0)})})}renderLabView(a){this.activeLesson;const t=this.currentLessons;this.currentGuardian==="html"||this.currentGuardian,a.innerHTML=`
        ...
        <div class="lab-toolbar">
          ...
          <div style="display: flex; align-items: center; gap: 0.75rem;">
            ...
            <select id="lab-challenge-selector" class="challenge-dropdown-select">
              ${t.map((i,n)=>{const r=this.completedLessons.has(i.id);return`
                  <option value="${n}" ${n===this.currentLessonIndex?"selected":""}>
                    ${r?"✅ ":"🐾 "}${i.title} (${i.level})
                  </option>
                `}).join("")}
            </select>
    ...
    `,a.querySelector("#lab-back-to-home-btn").addEventListener("click",()=>{this.switchView("home")}),a.querySelectorAll(".guardian-tab-btn").forEach(i=>{i.addEventListener("click",()=>{const n=i.getAttribute("data-guardian");this.switchGuardianInLab(n)})}),a.querySelector("#lab-challenge-selector").addEventListener("change",i=>{this.currentLessonIndex=parseInt(i.target.value,10),this.showHint=!1,this.lastTestResult=null,this.renderChallengeDetail(),this.loadActiveLesson()});const o=a.querySelector("#lab-sandbox-container");this.sandbox=new b(o),this.renderChallengeDetail(),this.loadActiveLesson()}switchGuardianInLab(a){this.currentGuardian=a,this.currentLessonIndex=0,this.showHint=!1,this.lastTestResult=null,this.renderLabView(document.getElementById("views-viewport"))}renderChallengeDetail(){const a=document.getElementById("lab-challenge-panel-content");if(!a)return;const t=this.activeLesson,s=this.currentGuardian==="html"?"var(--guardian-html)":this.currentGuardian==="css"?"var(--guardian-css)":"var(--guardian-js)";a.style.setProperty("--current-guardian-color",s),a.innerHTML=`
      <div class="challenge-detail-box">
        <!-- Metadatos superiores separados con aire -->
        <div class="challenge-top-meta">
          <span class="meta-badge" style="background: rgba(255,255,255,0.06); color: ${s}; border: 1px solid var(--border-subtle);">
            ${t.badge}
          </span>
          <span class="meta-badge level">
            Nivel ${t.level}
          </span>
          ${this.completedLessons.has(t.id)?`
        ...
        <h2 class="challenge-main-heading">${t.title}</h2>
        <p class="challenge-main-description">${t.description}</p>
        ...
        <div class="objectives-card">
          ...
          ${t.objectives.map(o=>`
            <div class="objective-item-row">
              <span class="objective-icon" style="color: ${s};">🐾</span>
              <span>${o}</span>
            </div>
          `).join("")}
        </div>
        ...
        <div class="theory-card">
          <strong style="color: #ffffff;">💡 Fundamento Teórico:</strong> ${t.theory}
        </div>
        ...
        ${this.lastTestResult?`
          <div class="test-feedback-card ${this.lastTestResult.passed?"success":"warning"}">
            ...
            ${this.lastTestResult.results.map(o=>`
              <div class="test-row">
                <span>${o.passed?"✅":"❌"}</span>
                <span>${o.desc}</span>
              </div>
        ...
        ${this.showHint?`
          <div class="hint-container">
            <strong style="color: #ffffff;">💡 Pista de ${t.badge}:</strong> ${t.hint||"Revisa la teoría y los objetivos arriba."}
          </div>
        `:""}
    ...
    `,a.querySelector("#lab-validate-btn").addEventListener("click",()=>{this.runValidation()}),a.querySelector("#lab-hint-btn").addEventListener("click",()=>{this.showHint=!this.showHint,this.renderChallengeDetail()}),a.querySelector("#lab-reset-btn").addEventListener("click",()=>{this.lastTestResult=null,this.loadActiveLesson(),this.renderChallengeDetail()}),a.querySelector("#lab-solution-btn").addEventListener("click",()=>{t.solutionCode&&this.sandbox.setCode({html:t.solutionCode.html||t.initialCode.html,css:t.solutionCode.css||t.initialCode.css,js:t.solutionCode.js||t.initialCode.js},!0)})}loadActiveLesson(){const a=this.activeLesson;a&&this.sandbox&&this.sandbox.setCode(a.initialCode,!0)}runValidation(){const a=this.activeLesson;if(!a||!this.sandbox)return;const t=this.sandbox.getCode(),s=m.evaluate(t,a.tests);if(this.lastTestResult=s,s.passed){this.completedLessons.add(a.id),localStorage.setItem("devfauna_completed",JSON.stringify([...this.completedLessons]));const o=document.getElementById("header-completed-count");o&&(o.textContent=this.completedLessons.size)}this.renderChallengeDetail()}}let u=!1;function p(){u||(u=!0,new h)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",p):p();
